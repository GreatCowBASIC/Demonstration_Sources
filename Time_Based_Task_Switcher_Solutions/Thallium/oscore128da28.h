...
''' AVR128DA28. Therefore, this version can serve as a template for your own 
''' adaptations to newer generation chips.
'''
'''--------------------------------------------------------------------------------------------------------------------------------
...
''' Updated 02/26 - the macros 'lock' and 'resume' have been added to the interrupt handler
...
macro initOS
  ...
  out CPU_CCP, r16
  ldi r16,36
  sts CLKCTRL_OSCHFCTRLA, r16
  ; Timer
  TCA0_SINGLE_INTCTRL = TCA_SINGLE_OVF_bm ; OVF enabled
  TCA0_SINGLE_PERL = osCMP0L  ; low(29999)  0x752F
  TCA0_SINGLE_PERH = osCMP0H  ; high(29999)
  TCA0_SINGLE_CTRLA = 0x07    ; CLKSEL DIV8, ENABLE
  asm showdebug               ; reset interrupt flag
...
macro setupTask(osTaskNbr, osLabel, osPriority, osFlags_bm)
if osTaskNbr = 0 then
...
;->
  ...
  in   r16, CPU_SPL                        ; get the Stackpointer
  in   r17, CPU_SPH                        ;
  ...
  sbr  r16, osFlags_bm  ; r16 OR osFlags_bm
  sts  osFlagsStore+osTaskNbr, r16
if osTaskNbr = 0 then
  osTaskPointer = 0     ; set to Task0
...
; Trigger task switching without increasing the osTickCount
  ...
  rcall REQUIRED_ISR_TCA0_CMP0  ; let another task run
end macro
...
; load the next weak up time to the timer store and sets the slumber flag
  ldi   XL,   Low(osTimerStore)     ; copy osTimerStore address to X
...
;-> (X now points to timer store address of current task)
  ldi   YL,   Low(osFlagsStore)     ; copy flags store task0 address to Y
...
; Suspends further searches in the lower priority levels if found.
    ...
    ldi   XL,   Low(osPrioTemp)   ; set X-Pointer to the beginning of the table of 'Task of equal priority'
    ldi   XH,   High(osPrioTemp)  ;
;->
    ldi   YL,   Low(osPriorStore)     ; set Y to the priority byte of Task0
    ldi   YH,   High(osPriorStore)    ;
    ldi   ZL,   Low(osFlagsStore)     ; set Z to the flags of Task0
    ldi   ZH,   High(osFlagsStore)    ;
...
; *************************************
    asm showdebug  ; reset Interuptflag
...
macro saveContext
  asm showdebug ; macro SAVE CONTEXT
    push  r0              ; save r0
    in    r0, CPU_SREG    ; save status register
    ...
    push  r30
    push  r31
    in    r0, CPU_SPL       ; store the the stack pointer in separate memory
    in    r1, CPU_SPH       ;
...
macro restoreContext
  asm showdebug ; macro RESTORE CONTEXT
    lds  r0, osStackTempL   ; read the address from memory and store to the stack pointer
    lds  r1, osStackTempH   ;
    out  CPU_SPL, r0        ; store to the Stack pointer - first low byte, then high byte
    out  CPU_SPH, r1        ;
    pop  r31
    pop  r30
    pop  r29
    ...
    out  CPU_SREG, r0     ; and then restore the status register
    pop  r0               ; restore r0
