...
#script
  ...
  If AVR Then
    ...
    End If
    If Bit(RXC1) Then
      'This needs to handle chip with either USART 1 or 2, so, need to isolate for chip like the mega32u4.
      If NoBit(RXC0) Then
        'Note: This assumes one USART called RXC1
        USARTHasData = "RXC1 = On"
      End If
      If Bit(RXC0) Then
        'Note: this assumes two USARTs called RXC0 and RXC1
...
Sub InitUSART
  ...
  #ENDIF
    ...
    #If USART_BAUD_RATE Then
      ...
      #IFDEF ONEOF( CHIP_MEGA32U4, CHIP_MEGA32U4 )

        U2X1 = U2X0_TEMP       'Set/Clear bit to double USART transmission speed
        UBRR1L = UBRRL_TEMP
        UBRR1H = UBRRH_TEMP
        RXEN1 = On
        TXEN1 = On
