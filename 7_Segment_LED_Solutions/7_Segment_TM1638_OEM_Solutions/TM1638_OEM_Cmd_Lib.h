...
'
'   TM1638 7-Segment OEM Display Library - TM1638_OEM_Cmd_Lib.h GreatCowBasic
'----------------------------------------------
...
''********************************************************************
' Required files TM1638_OEM_Message.h
'    2022/08/24   Initial version (copied from TM1637_OEM_Cmd_Lib.h

'   Rev 0.9.00
''********************************************************************
'Commands:    [See TM_1638_OEM Demo files for example] & TM1638 OEM CmdLib Ref.pdf
...
'  TM_KeyVal, TM_ButnVal         ' <- vars set by tmGetKey sub
'-------------------------------
...
//   #Ifdef Com_Anode ' Create regBuffer
//    Dim TM_RegBuf(16 +1) as Byte  ' TM1638 register display buffer, 8x 10bit
//   #endif
...
Table Seg7_Val '   Values for 7 segment display
...
'to Doo:
'     Accomodate different connect mode LED Common --> Seg or Grid ??
'     16 LED's.   Bicolor LED's

Sub tmSetLED (In LEDn, In LEDon)
      ' Code: LED set to Addr+1 & Reg value

      tmSndDig(TMdigVal, DigN)
End Sub
...
''    Tmp_1.7 = !Tmp_1.7   '< ok
''    Tmp_1.Seg_n = !Tmp_1.Seg_n  ' < error  'Toggle  disp buffer array  bit
''    TmpBit = Temp_1.Seg_n : TmpBit = !TmpBit : Temp_1.Seg_n = TmpBit '{workaround for array bitset var}
...
Dim FWLp as Byte
    For FWLp = 1 to 20
      If TM_KeyChk = On And TM_ButnVal = 0 then tmGetKey ' Key pressed?
