...
'
'   TM1638 7-Segment OEM Display Library - TM1638_OEM_Lib.h GreatCowBasic
'----------------------------------------------
...
' TooDoo:
'     Test/mod Sub tmSegBar
'     Test tmSetLED in ComAnode mode.
'
...
Dim Tmp_1, Tmp_2 as Byte    ' Temp, eg. var->array bitset, not var bitset, calc bitn
...
#Script
  ...
  IF NODEF(Com_Cathode) And NODEF(Com_Anode) then ' Set const "Com_Cathode"
    Com_Cathode=""
'    Warning "====== IF NODEF Cathode_or_Anode ======"
  END IF
  IF NODEF(TMDly) then
    TMdly=5       ' clk <-> DIO delay us (Close to this value)
  END IF
  IF NODEF(KeyMap) then
'    KeyMap=ButnMap1    ' with no button map can enable user to create custom map.
  END IF
  ' Set pin dir
#EndScript
...
'      TM_STB = 1
'      setTM1638_STB 1   '< initial hi state
     TMctrl = TMcmd_80h
